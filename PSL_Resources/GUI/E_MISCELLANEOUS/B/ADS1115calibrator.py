...
class acquirer():
    def __init__(self, parent):
        ...
        self.paused = False
        if self.I and len(
            sys.argv) == 1: self.I.__ignoreCalibration__()  # provide any argument to skip calibration erase
        self.DAC_CHAN = None
            ...
            if self.I.analogInputSources[a].gainEnabled:
                ...
                for b in range(8):
                    ...
                    if self.I.analogInputSources[a].__conservativeInRangeRaw__(rawval, 100):
                        v = self.I.analogInputSources[a].calPoly12(
                            np.average([self.I.__get_raw_average_voltage__(a) for x in range(averages)]))
                        self.ADC_VALUES[a][b].append(v)
                        self.ADC_ACTUALS[a][b].append(ADC_ACTUAL)
            else:
                if self.I.analogInputSources[a].__conservativeInRange__(ADC_ACTUAL):
                    v = self.I.analogInputSources[a].calPoly12(
                        np.average([self.I.__get_raw_average_voltage__(a) for x in range(10)]))
...
class AppWindow(QtGui.QMainWindow, ADS1115calibrator.Ui_MainWindow):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.setWindowTitle(
            self.I.generic_name + ' : ' + self.I.H.version_string.decode("utf-8") + ' : ' + time.ctime())
        ...
        for a in self.INPUTS:
            ...
            if self.I.analogInputSources[a].gainEnabled:
                for b in range(8):
                    ...
                    item = self.addLabel(name, col);
                    self.curves[a][b].setClickable(True);
                    self.curves[a][b].sigClicked.connect(functools.partial(self.selectItem, item))
            else:
                col = QtGui.QColor(random.randint(20, 255), random.randint(20, 255), random.randint(20, 255))
                name = '%s:1x' % (a)
                self.curves[a][0] = addCurve(self.plot, pen=pg.mkPen(col, width=1), name='%s:1x' % (a))
                item = self.addLabel(name, col);
                self.curves[a][0].setClickable(True);
                self.curves[a][0].sigClicked.connect(functools.partial(self.selectItem, item))
    ...
    def selectDir(self):
        ...
        dirname = QtGui.QFileDialog.getExistingDirectory(self, "Select a folder for dumping the calibration data.",
        ...
        if dirname:
            ...
            try:
                ...
                reply = QtGui.QMessageBox.question(self, 'Message', 'directory exists. overwrite?',
    ...
    def askBeforeQuit(self):
        global app
        reply = QtGui.QMessageBox.question(self, 'Warning', 'save and quit?', QtGui.QMessageBox.No,
        ...
        for a in self.INPUTS:
            if self.I.analogInputSources[a].gainEnabled:
                for b in range(8):
                    ...
                    np.savetxt(os.path.join(self.savedir, 'CALIB_%s_%dx.csv' % (a, self.I.gain_values[b])),
                               np.column_stack([actual, measured]))
            else:
    ...
    def upload(self):
        ...
        for a in self.A.ADC_VALUES:
            ...
            for b in keys:
                ...
                fitvals = np.polyfit(source.voltToCode12(self.A.ADC_VALUES[a][b][1:]), self.A.ADC_ACTUALS[a][b][1:],
                                     2)  # measured, actual, 2 degree.
