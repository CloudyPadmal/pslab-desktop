...
class AppWindow(QtGui.QMainWindow, wirelessTemplate.Ui_MainWindow, utilitiesClass):
    ...
    def addPlot(self, addr, param):
        ...
        if cls:
            ...
            if not self.active_device_counter:
                ...
                curves = [self.addCurve(self.plot, '%s[%s]' % (label[:10], cls.PLOTNAMES[a])) for a in
                          range(cls.NUMPLOTS)]
            else:
                if label:
            ...
            for a in range(cls.NUMPLOTS):
                ...
                action = QtGui.QCheckBox(
                    '%s' % (cls.PLOTNAMES[a]))  # self.curveMenu.addAction('%s[%d]'%(label[:12],a))
    ...
    def updateLogWindow(self):
		...
		'''
        #self.logs.setHtml(T) TODO: Removed QtWebKitWidgets
